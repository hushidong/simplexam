\documentclass[answer=false,countunit=section,sheetsize=A3,paperprint=double,scoretable]{simplexam}
...
%测试内容：
%测试answer的换行，\answer前面的字符如果是英文标点可以使\answer{B}放到行末
%如果\answer前面的字符是中文字符，则可能因为ctex产生空格使得中间会断行，
%则answer无法放到行末吗，而前面的判断中的测试题的表现说明中文字符也可以使放到行末。
